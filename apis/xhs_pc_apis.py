...
class XHS_Apis():
    ...
    def get_user_all_notes(self, user_url: str, cookies_str: str, max_notes: int = 0, proxies: dict = None):
        """
           获取用户所有笔记
           :param user_url: 用户链接
           :param cookies_str: 你的cookies
           :param max_notes: 限制获取数量，0 表示全部
           返回用户的笔记
        """
        cursor = ''
        note_list = []
        try:
            ...
            while True:
                ...
                if 'cursor' in res_json["data"]:
                    ...
                    break
                note_list.extend(notes)
                if max_notes > 0 and len(note_list) >= max_notes:
